            ...
            if (Regex.IsMatch(input, "[01234]"))
            {
                switch (input)
                {
                    ...
                    case "2":
                        ...
                        while (true)
                        {
                            _codingSession.StartTime = _helpers.EnterDatePrompt("Please enter date and time (e.g., 2024-08-10 14:30)");
                            if (_codingSession?.StartTime == null) break;

                            _codingSession.EndTime = _helpers.EnterDatePrompt("Please enter another date and time (e.g., 2024-08-10 14:30)");
                            if (_codingSession?.EndTime == null) break;
