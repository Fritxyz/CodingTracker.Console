            ...
            if (Regex.IsMatch(input, "[01234]"))
            {
                switch (input)
                {
                    ...
                    case "2":
                        ...
                        do
                        {
                            _codingSession.StartTime = Helpers.EnterDatePrompt("Please enter date and time (e.g., 2023-12-25 16:30:20)").Item1;
                            if (!_isPromptExited)
                                break;

                            Helpers.EnterDatePrompt("Please enter another date and time (e.g., 2023-12-25 16:30:20)", _codingSession.EndTime);
                            if (!_isPromptExited)
                                break;
                    ...
                    case "4":
                        ...
                        if (_codingSession.Id > 0)
                        {
                            do
                            {
                                _isPromptExited = Helpers.EnterDatePrompt("Please enter date and time (e.g., 2023-12-25 16:30:20)", _codingSession.StartTime);
                                if (!_isPromptExited)
                                    break;
