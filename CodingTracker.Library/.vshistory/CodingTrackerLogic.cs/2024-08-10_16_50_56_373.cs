...
namespace HabitLogger.Library
{
    ...
    {
        private static readonly CodingSession codingSession = new();
            ...
            if(Regex.IsMatch(input, "[01234]"))
            {
                switch (input)
                {
                    ...
                    case "1":
                        ...
                        bool isDateTimeParsed = DateTime.TryParse(startDateStr, out DateTime parsedDateTime);

                        if(isDateTimeParsed)
                        {
                            Console.WriteLine($"Date Parsed: {parsedDateTime}");
                        }
                        else
                        {
